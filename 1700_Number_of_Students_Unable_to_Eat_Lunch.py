...
ith sandwich in the stack (i = 0 is the top of the stack) and students[j]
...
class Solution:
    ...
    def countStudents(students: list[int], sandwiches: list[int]) -> int:
        cnt_ones = sum(students)
        cnt_zeroes = len(students) - cnt_ones
        for s in sandwiches:
            if s == 1:
                cnt_ones -= 1
            elif s == 0:
                cnt_zeroes -= 1
            if cnt_ones < 0 or cnt_zeroes < 0:
                return cnt_ones + cnt_zeroes + 1
        return cnt_ones + cnt_zeroes
